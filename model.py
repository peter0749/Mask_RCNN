...
class data_generator(Sequence):
    ...
    def __init__(self, dataset, config, shuffle=True, augment=True, random_rois=0, batch_size=1, detection_targets=False):
        ...
        self.imgaug_sequence = [iaa.Sequential(
            [
                ...
                # sometimes(iaa.PiecewiseAffine(scale=(0.005, 0.01))), # elastic distortion (!!!constly operations!!!)
                sometimes(iaa.PerspectiveTransform(scale=(0.01, 0.075))) # simmulate changes on z-axis ?!
            ],
            random_order=True
        ),
            iaa.Sequential(
            [
                ...
                sometimes(iaa.AddToHueAndSaturation((-10, 10))),
                sometimes(iaa.OneOf([
                    iaa.AdditiveGaussianNoise(loc=0, scale=(0.0, 0.05*255), per_channel=0.5), # add gaussian noise to images
                    iaa.SaltAndPepper(p=0.02)
                ])),
                iaa.Multiply((0.5, 1.5), per_channel=0.5),
                sometimes(iaa.ContrastNormalization((0.5, 1.2), per_channel=0.5)) # improve or worsen the contrast
            ],
            random_order=True
            )
        ]
    ...
    def on_epoch_end(self):
        if self.shuffle: np.random.shuffle(self.image_ids)

    def __len__(self):
        return int(np.ceil(float(len(self.image_ids))/self.batch_size))

    def __getitem__(self, batch_index):
        l_bound =  batch_index    * self.batch_size
        r_bound = (batch_index+1) * self.batch_size
        ...
        for b, image_index in enumerate(range(l_bound, r_bound)):
            ...
            if first: # initialize batchs
                ...
                if self.config.USE_MINI_MASK:
                    batch_gt_masks = np.zeros((self.batch_size, self.config.MINI_MASK_SHAPE[0], self.config.MINI_MASK_SHAPE[1], self.config.MAX_GT_INSTANCES))
                else:
...
class MaskRCNN():
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers, callbacks=[]):
        """Train the model.
